...
# dataFile = 'dataset26p.csv'
# data = pd.read_csv(dataFile,encoding='utf8')
# result = getKeywords_textrank4zh(data,50)
...
tok = StaticTokenizerEncoder(sample=result['keyword'].values, tokenize=lambda x: x.split(),
...
X = [tok.encode(text) for text in result['keyword'].values]  # 通过texts_to_sequences 这个dict可以将每个string的每个词转成数字
...
result['cat_id'] = result['cat'].factorize()[0]

Y = result['cat_id'].values
...
model = LSTMnet(output_size=len(cat_to_id),
                hidden_dim=50,
